...
void tulosta_tasot(puuosoitin);
Solmu *lisaa_alkio(Solmu *, puuosoitin);
Solmu *tulosta_alkiot(Solmu *);
void tyhjenna(puuosoitin);
...
while (lopeta != 1) {
	...
	printf("5) Tulosta puun avaimet tasoittain\n");
	printf("0) Lopeta\n");
	printf("Valintasi: ");
	scanf("%d", &valinta);
	...
	switch(valinta) {
		...
		case 5:
			printf("\n");
			printf("Puun asisältämät avaimet tasoittain: \n");
			tulosta_tasot(puu);
			break;
		default:
...
/*Tulostaa puun avaimet tasoittaen*/

void tulosta_tasot(puuosoitin juuri) {
...
if (juuri == NULL) {
	...
	if(alku == NULL) {printf("Tässä tulee NULL!\n");};

	while (alku != NULL) {
		alku = tulosta_alkiot(alku);
	}
}
}


/*Lisää lisää alkion listan viimeiseksi*/

Solmu  *lisaa_alkio(Solmu *pAlku, puuosoitin juuri) {

Solmu *ptr, *ptrUusi;

if ((ptrUusi = (Solmu *) malloc(sizeof(Solmu))) == NULL) {
	perror("Muistin varaaminen eoäonnistui! Ohejlma suljetaan.\n");
	exit(1);
}
...
/*Tulostaa tietyn rivin avaimet*/

Solmu *tulosta_alkiot(Solmu *p1) {

Solmu *ptr;
Solmu *p2 = NULL;

while (p1 != NULL) {
	puuosoitin avain = p1->alkio;

	if ((avain->vasen != NULL) && (avain->oikea != NULL)) {
		p2 =lisaa_alkio(p2, avain->vasen);
		p2 = lisaa_alkio(p2, avain->oikea);
	} else if ((avain->vasen != NULL) && (avain->oikea == NULL)) {
		p2 = lisaa_alkio(p2, avain->vasen);
	} else if ((avain->vasen == NULL) && (avain->oikea != NULL)) {
		p2 = lisaa_alkio(p2, avain->oikea);
	}
	printf("%d (%d)  ", avain->luku, avain->tila);
